...
function updateWeeklySummary() {
  if (!summaryCompleted || !summaryTargetsHit || !summaryProgress || !summaryPerfectWeeks) return;
...
function updateInsights() {
  if (!insightLongestStreak || !insightLongestStreakSubtext || !insightMostConsistent || !insightMostConsistentSubtext || !insightClosestTarget || !insightClosestTargetSubtext) {
    return;
  }
...
function renderHeatmap(habit) {
  ...
  const cells = days.map(day => {
    ...
    const classes = ["heatmap-cell", completed ? "completed" : "", day.isToday ? "today" : ""]
      .filter(Boolean)
      .join(" ");
...
function renderMonthlyCalendar() {
  if (!monthlySection || !monthlyGrid || !monthlySectionTitle) return;
...
function updateYearAchievements() {
  if (!yearAchievementsSection || !yearAchievementsSubtitle) return;
  ...
  if (achievementPerfectYearSubtext) {
  ...
  if (achievementMomentumYearSubtext) {
  ...
  if (achievementEliteConsistency) {
  ...
  if (achievementYearTotalsSubtext) {
...
function updateEmptyState() {
  ...
  if (!emptyState || !weeklySummary || !chartCard || !insightsCard || !habitList) return;
...
function showCelebrationToast(message) {
  if (!celebrationToast || !celebrationToastMessage) return;
...
function renderHabitDetail() {
  ...
  if (detailHabitName) detailHabitName.textContent = habit.name;
  if (detailHabitSubtitle) {
    detailHabitSubtitle.textContent = `Target: ${habit.targetDays} day${habit.targetDays === 1 ? "" : "s"} per week`;
  }
  if (detailWeekProgress) detailWeekProgress.textContent = `${habit.completedDays}/${habit.targetDays}`;
  if (detailStreak) detailStreak.textContent = `${habit.streak || 0}`;
  if (detailTotal) detailTotal.textContent = `${habit.total || 0}`;
  if (detailPerfectWeeks) detailPerfectWeeks.textContent = `${habit.perfectWeeks || 0}`;

  if (detailMonthLabel) detailMonthLabel.textContent = viewed.label;
  if (detailMonthReward) {
  ...
  if (detailPerfectMonths) detailPerfectMonths.textContent = `${yearStats.perfectMonths}`;
  if (detailStrongMonths) detailStrongMonths.textContent = `${yearStats.strongMonths}`;
  if (detailYearConsistency) detailYearConsistency.textContent = `${Math.round(yearStats.ratio * 100)}%`;

  if (detailReminderSummary) {
    detailReminderSummary.textContent = getReminderSummary(habit.reminder);
  }

  if (detailCompleteHabitBtn) {
    detailCompleteHabitBtn.textContent = getHabitCompletionButtonText(habit);
    detailCompleteHabitBtn.disabled = locked;
  }

  if (detailCompletionStatus) {
    detailCompletionStatus.textContent = locked
      ? "This habit is locked for the rest of the week."
      : (habit.doneToday
          ? "Completed today — you can still undo until the weekly target is reached."
...
function renderHabits() {
  ...
  sortedHabits.forEach((habit) => {
    ...
    if (habit.doneToday) li.classList.add("completed");
    ...
    li.innerHTML = `
      ...
      <div class="card-controls">
        <button type="button" class="drag-handle" title="Reorder" aria-label="Reorder habit">⋮⋮</button>
        <button type="button" class="delete-btn" onclick="event.stopPropagation(); openDeleteModalById('${habit.id}');">❌</button>
      </div>
    `;
...
function toggleHabitById(habitId) {
  ...
  if (!habit) return;
  if (isHabitLockedForWeek(habit)) return;
...
function openDeleteModalById(habitId) {
  const habit = habits.find(h => h.id === habitId);
  if (!habit || !deleteModal || !deleteModalText) return;
...
function openReminderModalForHabit(habitId) {
  const habit = habits.find(h => h.id === habitId);
  if (!habit || !reminderModal) return;

  reminderEditHabitId = habitId;

  if (reminderEnabled) reminderEnabled.checked = Boolean(habit.reminder?.enabled);
  if (reminderTime) reminderTime.value = habit.reminder?.time || "19:00";
...
function saveReminderSettings() {
  ...
  const selectedDays = reminderDaysGrid
    ? [...reminderDaysGrid.querySelectorAll(".day-pill.active")].map(btn => Number(btn.dataset.day))
    : [];
...
if (closeHabitDetailBtn) closeHabitDetailBtn.addEventListener("click", closeHabitDetail);
if (habitDetailBackdrop) habitDetailBackdrop.addEventListener("click", closeHabitDetail);
...
if (closeAppUpdateBtn) closeAppUpdateBtn.addEventListener("click", closeAppUpdateModal);
if (appUpdateBackdrop) appUpdateBackdrop.addEventListener("click", closeAppUpdateModal);

if (restoreLastWeekBtn) restoreLastWeekBtn.addEventListener("click", restoreLastWeekHabits);
if (dismissRestoreBannerBtn) dismissRestoreBannerBtn.addEventListener("click", dismissRestoreBannerForCurrentWeek);
...
if (habitForm) {
  habitForm.addEventListener("submit", function (event) {
    ...
    const targetDays = parseInt(targetDaysInput?.value || "5", 10);
...
if (confirmDeleteBtn) confirmDeleteBtn.addEventListener("click", confirmDeleteHabit);
if (deleteModalBackdrop) deleteModalBackdrop.addEventListener("click", closeDeleteModal);

document.addEventListener("keydown", function (event) {
  if (event.key === "Escape") {
    if (habitModal && !habitModal.classList.contains("hidden")) closeHabitModal();
    if (deleteModal && !deleteModal.classList.contains("hidden")) closeDeleteModal();
    if (habitDetailModal && !habitDetailModal.classList.contains("hidden")) closeHabitDetail();
    if (reminderModal && !reminderModal.classList.contains("hidden")) closeReminderModal();
    if (appUpdateModal && !appUpdateModal.classList.contains("hidden")) closeAppUpdateModal();
  }
});
